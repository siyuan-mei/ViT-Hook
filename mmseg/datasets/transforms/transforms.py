...
class GenerateEdge(BaseTransform):
    ...
    def transform(self, results: Dict) -> Dict:
        ...
        kernel = cv2.getStructuringElement(
            cv2.MORPH_RECT, (self.edge_width, self.edge_width)
        )
...
class ResizeShortestEdge(BaseTransform):
    ...
    def __init__(self, scale: Union[int, Tuple[int, int]], max_size: int) -> None:
        ...
        self.resize = TRANSFORMS.build(
            {"type": "Resize", "scale": 0, "keep_ratio": True}
        )
...
class BioMedical3DRandomCrop(BaseTransform):
    ...
    def __init__(
        self, crop_shape: Union[int, Tuple[int, int, int]], keep_foreground: bool = True
    ):
        super().__init__()
    ...
    def random_sample_location(self, seg_map: np.ndarray) -> dict:
        ...
        for c in all_classes:
            if c == 0:
                ...
                selected = all_locs[
                    np.random.choice(len(all_locs), target_num_samples, replace=False)
                ]
    ...
    def random_generate_crop_bbox(
        self, margin_z: int, margin_y: int, margin_x: int
    ) -> tuple:
        """Randomly get a crop bounding box.
    ...
    def generate_margin(self, results: dict) -> tuple:
        ...
        if self.keep_foreground:
            ...
            if selected_voxel is None:
                ...
                warnings.warn(
                    f"case does not contain any foreground classes"
                    f': {results["img_path"]}'
                )
...
class BioMedicalGaussianBlur(BaseTransform):
    ...
    def __repr__(self):
        ...
        repr_str += (
            "different_sigma_per_channel=" f"{self.different_sigma_per_channel}, "
        )
...
class BioMedicalRandomGamma(BaseTransform):
    ...
    def _adjust_gamma(self, img: np.array):
        ...
        def _do_adjust(img):
            ...
            img = (
                np.power(((img - img_min) / float(img_range + 1e-7)), gamma) * img_range
                + img_min
            )
            if retain_stats_here:
                img = img - img.mean()
...
class BioMedical3DPad(BaseTransform):
    ...
    @staticmethod
    def _to_pad(
        img: np.ndarray, pad_shape: Tuple[int, int, int], pad_val: Union[int, float] = 0
    ) -> np.ndarray:
...
class BioMedical3DRandomFlip(BaseTransform):
    ...
    def __repr__(self):
        repr_str = self.__class__.__name__
        repr_str += (
            f"(prob={self.prob}, axes={self.axes}, "
    ...
    def transform(self, results):
        ...
        if self.bgr_to_rgb:
            ...
            if self.additional_targets:
                for key, value in self.additional_targets.items():
                    if value == "image":
                        results[key] = cv2.cvtColor(
                            results["image2"], cv2.COLOR_RGB2BGR
                        )
