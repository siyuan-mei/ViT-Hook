...
class PolyLRRatio(PolyLR):
    ...
    def _get_value(self):
        ...
        param_groups_value = []
        for base_value, param_group in zip(
            self.base_values, self.optimizer.param_groups
        ):
            eta_min = (
                self.eta_min
                if self.eta_min_ratio is None
                else base_value * self.eta_min_ratio
            )
            step_ratio = (1 - 1 / (self.total_iters - self.last_step + 1)) ** self.power
            step_value = (param_group[self.param_name] - eta_min) * step_ratio + eta_min
