...
class RSImage:
    ...
    def __init__(self, image):
        self.dataset = (
            gdal.Open(image, gdal.GA_ReadOnly) if isinstance(image, str) else image
        )
        assert isinstance(self.dataset, gdal.Dataset), f"{image} is not a image"
    ...
    def read(self, grid: Optional[List] = None) -> np.ndarray:
        ...
        assert len(grid) >= 4, "grid must be a list containing at least 4 elements"
    ...
    def create_seg_map(self, output_path: Optional[str] = None):
        ...
        seg_map = driver.Create(output_path, self.width, self.height, 1, gdal.GDT_Byte)
    ...
    def create_grids(
        self, window_size: Tuple[int, int], stride: Tuple[int, int] = (0, 0)
    ):
        ...
        """
        assert len(window_size) == 2, "window_size must be a tuple of 2 elements"
...
class RSInferencer:
    ...
    def run(
        ...
        write_thread = threading.Thread(target=self.write, args=(image, output_path))
        write_thread.start()
        read_thread.join()
