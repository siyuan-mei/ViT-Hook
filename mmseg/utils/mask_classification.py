...
def seg_data_to_instance_data(ignore_index: int, batch_data_samples: SampleList):
...
class MatchMasks:
    ...
    def __init__(
        ...
        assert assigner is not None, (
            "'assigner' in decode_head.train_cfg" "cannot be None"
        )
    ...
    def get_targets(
        ...
        for i in range(batch_size):
            labels, mask_targets, mask_weights = self._get_targets_single(
                cls_scores[i], mask_preds[i], batch_gt_instances[i]
            )
    ...
    def _get_targets_single(
        self, cls_score: Tensor, mask_pred: Tensor, gt_instances: InstanceData
        ...
        matched_quiery_inds, matched_label_inds = self.assigner.assign(
            pred_instances=sampled_pred_instances, gt_instances=sampled_gt_instances
        )
        labels = gt_labels.new_full(
            (self.num_queries,), self.num_classes, dtype=torch.long
        )
