...
class DepthMetric(BaseMetric):
    ...
    def process(self, data_batch: dict, data_samples: Sequence[dict]) -> None:
        ...
        for data_sample in data_samples:
            ...
            if not self.format_only:
                gt_depth = data_sample["gt_depth_map"]["data"].squeeze().to(pred_label)
            ...
            if self.output_dir is not None:
                ...
                output_mask = pred_label.cpu().numpy() * self.depth_scale_factor

                cv2.imwrite(
                    png_filename,
                    output_mask.astype(np.uint16),
    ...
    def _get_eval_mask(self, gt_depth: Tensor):
        ...
        valid_mask = torch.logical_and(
            gt_depth > self.min_depth_eval, gt_depth < self.max_depth_eval
        )
    ...
    def _calc_all_metrics(gt_depth, pred_depth):
        ...
        log10 = torch.mean(torch.abs(torch.log10(pred_depth) - torch.log10(gt_depth)))
        silog = torch.sqrt(
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        for gt_depth, pred_depth in results:
            for key, value in self._calc_all_metrics(gt_depth, pred_depth).items():
