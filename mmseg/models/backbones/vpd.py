...
def register_attention_control(model, controller):
    ...
    def ca_forward(self, place_in_unet):
        ...
        def forward(x, context=None, mask=None):
            ...
            q, k, v = (
                tensor.view(tensor.shape[0] * h, tensor.shape[1], tensor.shape[2] // h)
                for tensor in [q, k, v]
            )
    ...
    def register_recr(net_, count, place_in_unet):
        ...
        if hasattr(net_, "children"):
            return sum(
                register_recr(child, 0, place_in_unet) for child in net_.children()
            )
        return count

    cross_att_count = sum(
        register_recr(net[1], 0, place)
        for net, place in [
            (
                (child, "down")
                if "input_blocks" in name
                    ...
                    else (child, "mid") if "middle_block" in name else (None, None)
                )
            )  # Default case
...
class UNetWrapper(nn.Module):
    ...
    def __init__(
        ...
        assert has_ldm, (
            "To use UNetWrapper, please install required "
    ...
    def forward(self, x, timesteps=None, context=None, y=None, **kwargs):
        ...
        hs, emb, out_list = self._unet_forward(
            x, timesteps, context, y, diffusion_model
        )
        if self.use_attn:
    ...
    def _append_attn_to_output(self, out_list):
        ...
        attn64 = (
            torch.stack(attns[self.size64]).mean(0)
...
class TextAdapter(nn.Module):
    ...
    def __init__(self, text_dim=768):
        ...
        self.fc = nn.Sequential(
            nn.Linear(text_dim, text_dim), nn.GELU(), nn.Linear(text_dim, text_dim)
        )
...
class VPD(BaseModule):
    ...
    def __init__(
        ...
        assert has_ldm, (
            "To use VPD model, please install required packages"
        ...
        if class_embed_select:
            class_embeddings = torch.cat(
                (class_embeddings, class_embeddings.mean(dim=0, keepdims=True)), dim=0
            )
