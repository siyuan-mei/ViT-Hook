...
class ViTHook(TIMMVisionTransformer):
    def __init__(
        self,
        pretrained=None,
        pretrain_size=224,
    ...
    def forward(self, image):
        ...
        pos_embed = self._reshape_pos_embed(
            self.pos_embed[:, 1:], original_size=self.pretrain_size, size=(H, W)
        )
        vit_feature = self.pos_drop(vit_feature + pos_embed)
        ...
        if image.shape[2:] != self.crop_size:
            level_pos_embed = self._reshape_pos_embed(
                self.level_pos_embed, original_size=self.crop_size, size=(H, W)
            )
        else:
            level_pos_embed = self.level_pos_embed

        c2, c3, c4 = self._add_pos_level_embed(
            c2, c3, c4, level_pos_embed, image.shape[2:]
        )
        c = torch.cat([c2, c3, c4], dim=1)

        if not isinstance(self.hooks, nn.Identity):
