...
class EncoderDecoder(BaseSegmentor):
    ...
    def _init_decode_head(self, decode_head: ConfigType) -> None:
        """Customize Initialize ``decode_head``"""
        if self.neck is not None:
            if hasattr(self.neck, "stage_dims"):
                decode_head.update({"stage_dims": self.neck.stage_dims})
            if hasattr(self.neck, "feature_sizes"):
                decode_head.update({"feature_sizes": self.neck.feature_sizes})
