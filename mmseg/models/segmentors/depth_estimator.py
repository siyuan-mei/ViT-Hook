...
class DepthEstimator(EncoderDecoder):
    ...
    def extract_feat(
        self, inputs: Tensor, batch_img_metas: Optional[List[dict]] = None
    ) -> Tensor:
        """Extract features from images."""

        if (
            getattr(self.backbone, "class_embed_select", False)
            and isinstance(batch_img_metas, list)
            and "category_id" in batch_img_metas[0]
        ):
    ...
    def slide_flip_inference(
        self, inputs: Tensor, batch_img_metas: List[dict]
    ) -> Tensor:
    ...
    def postprocess_result(
        self, depth: Tensor, data_samples: OptSampleList = None
    ) -> SampleList:
        ...
        for i in range(batch_size):
            ...
            data_samples[i].set_data({"pred_depth_map": PixelData(**{"data": i_depth})})
