...
class BaseMatchCost:
    ...
    @abstractmethod
    def __call__(
        self, pred_instances: InstanceData, gt_instances: InstanceData, **kwargs
    ) -> Tensor:
        """Compute match cost.
...
class ClassificationCost(BaseMatchCost):
    ...
    def __call__(
        ...
        Returns:
            Tensor: Match Cost matrix of shape (num_preds, num_gts).
        """
        assert hasattr(pred_instances, "scores"), "pred_instances must contain 'scores'"
        assert hasattr(gt_instances, "labels"), "gt_instances must contain 'labels'"
...
class DiceCost(BaseMatchCost):
    ...
    def __call__(
        ...
        Returns:
            Tensor: Match Cost matrix of shape (num_preds, num_gts).
        """
        assert hasattr(pred_instances, "masks"), "pred_instances must contain 'masks'"
        assert hasattr(gt_instances, "masks"), "gt_instances must contain 'masks'"
