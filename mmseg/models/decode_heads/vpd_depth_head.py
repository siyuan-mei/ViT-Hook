...
class VPDDepthDecoder(BaseModule):
    ...
    def __init__(
        ...
        init_cfg: Optional[Union[Dict, List[Dict]]] = dict(
            type="Normal", std=0.001, layer=["Conv2d", "ConvTranspose2d"]
        ),
    ):
...
class VPDDepthHead(BaseDecodeHead):
    ...
    def __init__(
        ...
        init_cfg=dict(type="TruncNormal", std=0.02, layer=["Conv2d", "Linear"]),
    ):
        ...
        self.conv2 = nn.Conv2d(in_channels[1], in_channels[1], 3, stride=2, padding=1)
    ...
    def forward(self, x):
        x = [x[0], x[1], torch.cat([x[2], F.interpolate(x[3], scale_factor=2)], dim=1)]
        x = torch.cat([self.conv1(x[0]), self.conv2(x[1]), x[2]], dim=1)
        x = self.conv_aggregation(x)

        x = x[
            :, :, : x.size(2) - self.fmap_border[0], : x.size(3) - self.fmap_border[1]
        ].contiguous()
        x = self.decoder(x)
    ...
    def loss_by_feat(
        self, pred_depth_map: Tensor, batch_data_samples: SampleList
    ) -> dict:
        ...
        for loss_decode in losses_decode:
            if loss_decode.loss_name not in loss:
                loss[loss_decode.loss_name] = loss_decode(pred_depth_map, gt_depth_map)
            else:
                loss[loss_decode.loss_name] += loss_decode(pred_depth_map, gt_depth_map)
