...
class SideAdapterNetwork(nn.Module):
    ...
    def encode_feature(
        ...
        if self.pos_embed.shape[1] != x.shape[1]:
            ...
            pos_embed = (
                resize(
                    self.pos_embed.reshape(1, ori_h, ori_w, -1).permute(0, 3, 1, 2),
                    size=hwshape,
                    mode="bicubic",
                    align_corners=False,
                )
        ...
        for index, block in enumerate(self.encode_layers, start=1):
            x = block(x)
            if (
                index < len(self.fusion_index)
                and index == self.fusion_index[fused_index]
            ):
                x = self.fuse_clip(
                    fused_index, x, clip_features[fused_index][0], hwshape, L
                )
                fused_index += 1
            x_query = x[:, :-L, ...]
            x_feat = (
    ...
    def _build_attn_biases(self, attn_biases, target_shape):
        ...
        for attn_bias in attn_biases:
            ...
            attn_bias = F.adaptive_max_pool2d(
                attn_bias.reshape(n, num_head * num_sos, h, w), output_size=target_shape
            )
...
class SideAdapterCLIPHead(BaseDecodeHead):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert san_cfg.num_queries == maskgen_cfg.sos_token_num, (
            "num_queries in san_cfg should be equal to sos_token_num " "in maskgen_cfg"
        )
        del self.conv_seg
    ...
    def init_weights(self):
        ...
        if (
            ...
            and self.init_cfg.get("type") == "Pretrained_Part"
        ):
    ...
    def forward(self, inputs: Tuple[Tensor], deep_supervision_idxs) -> Tuple[List]:
        """Forward function.
    ...
    def predict(
        ...
        return self.predict_by_feat([mask_props[-1], mask_logits[-1]], batch_img_metas)

    def predict_by_feat(
        self, seg_logits: List[Tensor], batch_img_metas: List[dict]
    ) -> Tensor:
    ...
    def loss(
        ...
        batch_gt_instances = seg_data_to_instance_data(
            self.ignore_index, batch_data_samples
        )

        # forward
        all_mask_props, all_mask_logits = self.forward(x, self.deep_supervision_idxs)

        # loss
        losses = self.loss_by_feat(all_mask_logits, all_mask_props, batch_gt_instances)
    ...
    def loss_by_feat(
        ...
        for i in range(num_dec_layers):
            ...
            (labels, mask_targets, mask_weights, avg_factor) = (
                self.match_masks.get_targets(
                    cls_scores, mask_preds, batch_gt_instances_list[i]
                )
            )
            cls_scores = cls_scores.flatten(0, 1)
            labels = labels.flatten(0, 1)
            num_total_masks = cls_scores.new_tensor([avg_factor], dtype=torch.float)
            ...
            for loss_decode in losses_decode:
                if "loss_cls" in loss_decode.loss_name:
                    if loss_decode.loss_name == "loss_cls_ce":
                        loss[loss_decode.loss_name] = loss_decode(cls_scores, labels)
                    else:
                        assert False, (
                            "Only support 'CrossEntropyLoss' in" " classification loss"
                        )
                    ...
                    if mask_targets.shape[0] == 0:
                        ...
                        assert False, (
                            "Only support 'CrossEntropyLoss' and"
