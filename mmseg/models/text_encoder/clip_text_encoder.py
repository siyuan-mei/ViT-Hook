...
class CLIPTextEncoder(BaseModule):
    ...
    def __init__(
        ...
        assert (
            dataset_name is not None or vocabulary is not None
        ), "text_encoder required either 'dataset_name' or 'vocabulary'"
        assert (
            dataset_name is None or vocabulary is None
        ...
        self.ln_final = build_norm_layer(norm_cfg, embed_dims, postfix="_final")[1]
        ...
        self.cat_bg = cat_bg
        if self.cat_bg:
            self.bg_embed = nn.Parameter(torch.randn(1, self.text_projection.shape[1]))
    ...
    def encode_text(self, text, normalize=False):
        ...
        x = self.token_embedding(text.to(embed_device))  # [batch_size, n_ctx, d_model]
