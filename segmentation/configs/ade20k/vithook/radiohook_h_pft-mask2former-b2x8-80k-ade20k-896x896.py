...
model = dict(
    ...
    backbone=dict(
        ...
        use_abs_pos=True,
        active_block_indices=(7, 15, 23, 31),  # (3, 7, 11, 15, 19, 23, 27, 31)
        drop_path_rate=0.5,
    ...
    train_cfg=dict(type="IterBasedTrainLoop", val_interval=5000),
    test_cfg=dict(
        type="TestLoop", mode="slide", crop_size=crop_size, stride=(512, 512)
    ),
)
train_cfg = dict(val_interval=5000)
optim_wrapper = dict(
    type="AmpOptimWrapper",
    optimizer=dict(
        type="AdamW", lr=2.0e-5, betas=(0.9, 0.999), weight_decay=0.05, eps=1e-8
    ),
    dtype="float32",
    paramwise_cfg=dict(custom_keys={"backbone.blocks": dict(lr_mult=0.5)}),
)
param_scheduler = [
