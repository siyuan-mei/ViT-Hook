...
optim_wrapper = dict(
    ...
    optimizer=dict(
        type="AdamW", lr=6e-5, betas=(0.9, 0.999), weight_decay=0.05, eps=1e-8
    ),
    dtype="float16",
    ...
    paramwise_cfg=dict(
        ...
        # custom_keys={
        #     'backbone.blocks': dict(lr_mult=0.5),
        #      'backbone.patch_embed': dict(lr_mult=0.5),
        #      'backbone.pos_embed': dict(lr_mult=0.5)},
    ),
)
param_scheduler = [
