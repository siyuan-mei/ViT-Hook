...
class CoDETR(BaseDetector):
    ...
    def __init__(
        ...
        if query_head is not None:
            query_head.update(
                train_cfg=(
                    train_cfg[head_idx]
                    if (train_cfg is not None and train_cfg[head_idx] is not None)
                    else None
                )
            )
        ...
        for i in range(len(roi_head)):
            if roi_head[i]:
                rcnn_train_cfg = (
                    train_cfg[i + head_idx].rcnn
                    if (train_cfg and train_cfg[i + head_idx] is not None)
                    else None
                )
        ...
        for i in range(len(bbox_head)):
            if bbox_head[i]:
                bbox_head[i].update(
                    train_cfg=(
                        train_cfg[i + head_idx + len(self.roi_head)]
                        if (
                            train_cfg
                            and train_cfg[i + head_idx + len(self.roi_head)] is not None
                        )
                        else None
                    )
                )
                bbox_head[i].update(
                    test_cfg=test_cfg[i + head_idx + len(self.roi_head)]
                )
    ...
    def with_roi_head(self):
        """bool: whether the detector has a RoI head"""
        return (
            hasattr(self, "roi_head")
            and self.roi_head is not None
        ...
        if self.with_pos_coord and len(positive_coords) > 0:
            for i in range(len(positive_coords)):
                bbox_losses = self.query_head.loss_aux(
                    x, positive_coords[i], i, batch_data_samples
                )
